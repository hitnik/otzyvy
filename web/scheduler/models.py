...
class RepeatableJob(ScheduledTimeMixin, BaseJob):
    ...
    def time_shift(self):
        scheduled_date = self.schedule_time_utc()
        local_tz = pytz.timezone(settings.TIME_ZONE)
        utc_now = datetime.datetime.utcnow().replace(tzinfo=pytz.utc)
        while scheduled_date < utc_now:
            scheduled_date = scheduled_date + datetime.timedelta(seconds=self.interval_seconds())
            self.scheduled_time = scheduled_date.astimezone(local_tz)
            utc_now = datetime.datetime.utcnow().replace(tzinfo=pytz.utc)
    ...
    def schedule(self):
        if self.is_schedulable() is False:
            return False
        kwargs = {
            'scheduled_time': self.time_shift(),
